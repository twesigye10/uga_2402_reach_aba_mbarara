...
df_analysis_wide_refugee <- df_roster_analysis_refugee$results_table %>% 
...
# openxlsx::write.xlsx(df_analysis_wide_refugee, "outputs/test_wide_format.xlsx")
...
df_analysis_wide_refugee_reodered <- df_analysis_wide_refugee %>%
    relocate(any_of(reordered_columns), .after = "analysis_var_value") %>% 
    relocate(analysis_type, .after = "analysis_var_value")

cols_for_num_pct_formatting <- df_analysis_wide_refugee_reodered %>% 
...
df_to_extract_header = df_analysis_wide_refugee_reodered %>% 
...
addWorksheet(wb_refugee, sheetName="Analysis")


# addStyle(wb_refugee, sheet = "Analysis", hs1, rows = 1, cols = 3:34, gridExpand = TRUE)

# header showing results headings
writeData(wb_refugee, sheet = "Analysis", df_extracted_header %>% head(1), startCol = 2, 
          startRow = 3, headerStyle = hs2, colNames = FALSE, 
          borders = "all", borderColour = "#000000", borderStyle = "thin")

setColWidths(wb = wb_refugee, sheet = "Analysis", cols = 1, widths = 70)
setColWidths(wb = wb_refugee, sheet = "Analysis", cols = 2, widths = 10)
setColWidths(wb = wb_refugee, sheet = "Analysis", cols = 3:90, widths = 8)

# split variables to be written in different tables with in a sheet
sheet_variables_data <- split(df_analysis_wide_refugee_reodered, factor(df_analysis_wide_refugee_reodered$analysis_var, levels = unique(df_analysis_wide_refugee_reodered$analysis_var)))
...
for (i in 1:length(sheet_variables_data)) {
    ...
    writeData(wb_refugee, sheet = "Analysis", get_question, startCol = 1, startRow = previous_row_end + 1)
    writeData(wb_refugee, sheet = "Analysis", get_qn_type, startCol = 2, startRow = previous_row_end + 1)
    addStyle(wb_refugee, sheet = "Analysis", hs2, rows = previous_row_end + 1, cols = 1:2, gridExpand = TRUE)
    
    current_data_length <- max(current_variable_data$row_id) - min(current_variable_data$row_id)
    
    writeData(wb = wb_refugee, 
              sheet = "Analysis", 
...
# hide grid lines
# showGridLines(wb_refugee,  "Analysis", showGridLines = FALSE)  

# freeze pane
freezePane(wb_refugee, "Analysis", firstActiveRow = 4, firstActiveCol = 3)
