...
create_composites_loop_roster_refugee <- function(input_df) {
    input_df %>%
        mutate(i.hh_members_age_group = case_when(age <= 2 ~ "age_0_2",
               ...
               i.disability_age_group = case_when(age <13 ~ "age_5_12",
                                                  age <19 ~ "age_13_18",
                                                  age <25 ~ "age_19_24",
                                                  age <=59 ~ "age_25_59",
                                                  age >= 60  ~ "age_greater_59"),
               int.disability = unite(col = starts_with("vulnerability_"), remove = FALSE, sep = " : ", na.rm = TRUE),
               i.hh_disability_prevalence =  case_when(str_detect(string = int.disability, pattern = "yes_a_lot_of_difficulty|cannot_do_at_all") ~ "yes_disability",
                                                       !str_detect(string = int.disability, pattern = "yes_a_lot_of_difficulty|cannot_do_at_all") ~ "no_disability"),
